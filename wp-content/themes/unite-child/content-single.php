...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="entry-content">
		<?php the_content();
		    ...
		    if (get_post_type(get_the_ID()) == 'films') {
		        $terms = listAllTaxonomies(get_the_ID(),['genre','countries','years','actors']);

		        foreach ($terms as $key => $term) {
                    echo '<p>'.ucfirst($key).': ' . $term . '</p>';
                }

                $has_release_date = metadata_exists( 'post', get_the_ID(), 'release_date' );
                $has_ticket_price = metadata_exists( 'post', get_the_ID(), 'ticket_price' );
                if ($has_ticket_price){
                    echo '<p>Ticket Price: ' . get_post_meta( get_the_ID(), 'ticket_price', true ) . '</p>';
                }
                if ($has_release_date){
                    echo '<p>Release Date: ' . get_post_meta( get_the_ID(), 'release_date', true ) . '</p><br>';
                }
            }
